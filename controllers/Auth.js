...
async function verifyOtp(req, res) {
    try {
        ...
        if (isOtpExisting && (await compare(req.body.otp, isOtpExisting.otp))) {
            await Otp.findByIdAndDelete(isOtpExisting._id)
...
async function resetPassword(req, res) {
    try {
        ...
        if (isResetTokenExisting && isResetTokenExisting.expiresAt > new Date() && (await compare(req.body.token, isResetTokenExisting.token))) {

            // deleting the password reset token
            await PasswordResetToken.findByIdAndDelete(isResetTokenExisting._id)

            // resets the password after hashing it
            await User.findByIdAndUpdate(isExistingUser._id, { password: await hash(req.body.password, 10) })
...
export default {
    signup, login, verifyOtp, checkAuth, logout, resetPassword, resendOtp, forgotPassword
}